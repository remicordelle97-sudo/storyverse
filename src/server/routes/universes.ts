...
// Delete a universe and all its data — admin only. Users used to be
// able to delete their own universes for self-service cleanup of
// failed builds, but the recovery is now handled by support: failed
// universes already don't count toward the quota, so the loss-of-
// data risk from a stray click outweighs the convenience. Admin
// reset (POST /api/admin/users/:userId/reset) and the
// UniverseManager admin page cover the remaining cleanup paths.
